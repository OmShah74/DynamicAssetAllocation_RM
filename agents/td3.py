...
class TD3:
    def __init__(self, latent_dim, action_dim, name, load_weights=False):
        ...
        # Build all networks (main and target) by calling them with dummy data.
        # This is necessary before you can copy weights.
        dummy_embedding = tf.random.normal([1, latent_dim])
        dummy_action = self.actor(dummy_embedding)  # Get a sample action to build critics

        # Build main networks
        self.critic1([dummy_embedding, dummy_action])
        self.critic2([dummy_embedding, dummy_action])

        # Build the target networks by calling them with dummy data
        ...
        # Now that all networks are built, copying weights will work
    ...
    def update(self, z, a, r, z_prime, not_done):
        ...
        self.critic_optimizer.apply_gradients(
            zip(critic_grad, self.critic1.trainable_variables + self.critic2.trainable_variables))
